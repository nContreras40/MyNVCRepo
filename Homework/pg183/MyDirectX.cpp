 ...
 * GAME 2332-001
 * PG 183 p 01
 * Recreate the program described at the end of chapter 6, page 183.
 * 99% of the code is from the book.
 */
...
void Sprite_Draw_Frame(LPDIRECT3DTEXTURE9 texture, int destx, int desty, int framenum, int framew, int frameh, int columns)
{
	...
	D3DXVECTOR3 position2( (float)destx+100, (float)desty+100, 0 );

	RECT rect2;
 	rect2.left = (framenum % columns) * framew;
	rect2.top = (framenum / columns) * frameh;
	rect2.right = rect.left + framew;
	rect2.bottom = rect.top + frameh;

   D3DXVECTOR3 position3( (float)destx+300, (float)desty+300, 0 );

	RECT rect3;
 	rect3.left = (framenum % columns) * framew;
	rect3.top = (framenum / columns) * frameh;
	rect3.right = rect.left + framew;
	rect3.bottom = rect.top + frameh;

	spriteobj->Draw( texture, &rect, NULL, &position, white);
   spriteobj->Draw( texture, &rect2, NULL, &position2, white);
   spriteobj->Draw( texture, &rect2, NULL, &position3, white);
}
